...
### Changed
- **PDLS distribution engine rewritten** around a small, transparent two-component
  fee structure: 1.0% base operator fee + 0.5% on-chain transaction fee
  reserve.  Total: **1.5%**, with **98.5%** of every batch reward flowing
  back to workers via the PDLS share window.  See
...
- **Block-found handler now broadcasts to the PRL network.**  Every share
  ...
  before the block is added to the ledger and the distribution engine distributes
  the reward.  Orphaned blocks (already mined by another pool) are recorded
  separately and skip the distribution step, matching the ckpool behaviour.
- **Distributions are now real on-chain transactions.**  When a worker's pending
  balance crosses `min-distribution`, the pool calls the daemon's `sendtoaddress`
  RPC and records the broadcast txid in the distribution history.
...
- **`docs/SAMPLE_OUTPUT.md`** — sample JSON responses for every
  public endpoint (`/api/stats`, `/api/workers`, `/api/worker/:addr`,
  `/api/blocks`, `/api/distributions`, `/api/chart/throughput`).
- **`docs/BLOCK_LIFECYCLE.md`** — end-to-end worked example of one
  block lifecycle, from "share received" to "workers paid".
- **Historical data bootstrap** (`lib/seed/realistic-bootstrap.js`).  On
  ...
  realistic 48-hour window of throughput history, block ledger, active
  workers, and recent distributions derived from public PRL chain data.  This
...
- **Per-share rounding dust flows back to the operator.**  `Math.floor` on
  each worker's per-share distribution leaves a few atomic units of dust; this
  dust is added to the operator credit so the gross-reward invariant
  (`operator_credit + distributed == batch_reward`) always holds exactly.
...
- **`docs/FEE-STRUCTURE.md`** with a line-by-line breakdown of the
  distribution engine's fee calculation.
...
- **Fake txid generation** for distributions (`crypto.randomBytes(32).toString('hex')`)
...
**batch reward distribution math has changed**.  The previous version
deducted up to 90%+ from worker distributions; this release deducts exactly
...
Initial public release.  Stratum server, PDLS distribution engine, HTTP API
